...
Browser → Amplify (static SPA)
         ...
         (BSE India API)     (XGBoost, TFT)        (Gemma 3 4B IT)
              ↓
         DynamoDB (cache)
```
...
- **ML Inference (SageMaker)**: XGBoost and TFT models run on SageMaker Serverless Inference Endpoints — auto-scales to zero when idle
...
- **AI Technical Horizon Chart** — 30-day historical prices, weekends excluded
...
- XGBoost: returns `null` if SageMaker endpoint unavailable (frontend handles nullable fields)
